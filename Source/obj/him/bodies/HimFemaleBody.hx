...
class HimFemaleBody extends HimMaleBody {
	...
	public var breastOffset:Array<ASAny> = [2, 152, 302, 452, 602];
	...
	override public function loadDataPairs(param1:Array<ASAny>) {
		...
		for (_tmp_ in param1) {
			...
			switch (_loc2_[0]) {
				case "hisSkin":
					this.setSkinx(ASCompat.toInt(_loc2_[1]));

				case "hisBreasts":
					this.setBreasts(Std.int(Math.max(1, Math.min(149, ASCompat.toInt(_loc2_[1])))));
	...
	override public function shuffle() {
		...
		var _loc2_:AlphaRGBObject = PaletteUtils.getHue(Math.random() * 360, PaletteUtils.midRandom(),
			Math.random() > 0.6 ? ASCompat.toNumber(PaletteUtils.midRandom()) : ASCompat.toNumber(null));
	...
	public function setChildrenFrame(param1:DisplayObjectContainer, param2:UInt) {
		...
		var i:UInt = 0;
		while (i < numChildren) {
			if (Std.is(target.getChildAt(i), MovieClip)) {
