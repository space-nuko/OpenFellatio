...
class Main extends Sprite {
	...
	public function tick(param1:Event) {
		var now = Timer.stamp();
		var deltaTime = (now - __prevTime) / (stage.frameRate / 60) * G.timeScale;
		__prevTime = now;
		...
		if (G.gameRunning) {
			if (!G.gamePaused) {
				...
				G.animationControl.stepAnimation(deltaTime);
				G.dialogueControl.update();
			}
			G.soundControl.tick(deltaTime);
			G.characterControl.tick(deltaTime);
		}
	}
